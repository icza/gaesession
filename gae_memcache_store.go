...
func (s *memcacheStore) Add(sess session.Session) {
	s.mux.Lock()
	defer s.mux.Unlock()
...
func (s *memcacheStore) setMemcacheSession(sess session.Session) (success bool) {
	item := &memcache.Item{
...
func (s *memcacheStore) Remove(sess session.Session) {
	s.mux.Lock()
	defer s.mux.Unlock()
